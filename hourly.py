...
def oid(o):
    raw = "|".join(
        str(o.get(k, "")).strip().lower()
        for k in ("source", "url", "title", "company")
    )
...
def exists(offer_id):
    rows = request(
        f"offers?id=eq.{offer_id}&select=id"
    )
    return bool(rows)
...
def save(o):
    ...
    request(
        "offers?on_conflict=id",
        "POST",
        [row],
        "resolution=merge-duplicates,return=minimal"
    )

    if not was and row["decision"] in (
        "auto_apply",
        "manual_review"
    ):
        typ = (
            "apply_now"
            if row["decision"] == "auto_apply"
            else "review_now"
        )
